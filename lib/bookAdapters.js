...
export async function addRecord({ bookId, sessionId, type, content, imageUrl }) {
  const res = await fetch("/api/records", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ bookId, sessionId, type, content, imageUrl }),
  });
  const data = await res.json();
  if (!res.ok) {
    handleUnauthorized(res);
...
// Uploads a drawing (from the in-app drawing pad) or a photo attached to a
// record. Separate bucket from book covers, same pattern.
export async function uploadRecordImage(blob) {
  const form = new FormData();
  form.append("image", blob, "record.png");
  const res = await fetch("/api/records/image", { method: "POST", body: form });
  const data = await res.json();
  if (!res.ok) {
    handleUnauthorized(res);
    throw new Error(data?.error || "record image upload failed");
  }
  return data; // { url }
}

export async function addReadingSession(bookId, date) {
  const res = await fetch("/api/reading-sessions", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ bookId, date }),
  });
  const data = await res.json();
  if (!res.ok) {
    handleUnauthorized(res);
  ...
  return data; // { sessions } — the book's full, renumbered session list
}
...
export async function updateBook(id, { title, author, series, tags }) {
  const res = await fetch(`/api/books/${id}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ title, author, series, tags }),
  });
  const data = await res.json();
  if (!res.ok) {
    handleUnauthorized(res);
...
export const bookAdapters = { identifyBooksFromImage, searchBook, saveBooks, addRecord, addReadingSession, identifyCover, uploadCover, updateBook, deleteBook, uploadRecordImage };
